 ...
 *                                         (costs money, needs
 *                                         ANTHROPIC_API_KEY)
 *   npm run eval:schedule -- --selftest — dry run: feeds a hand-built
 *                                         schedule (one valid, one
 *                                         deliberately broken) through the
 *                                         SAME assertion function via a
 *                                         fake client, proving the
 *                                         assertions actually fire before
 *                                         spending a cent on the real API.
 */
...
// re-failing every run over cosmetic wording. WHY 'oil change' needs the
// "not filter" exclusion: without it, "oil filter" would also satisfy the
// 'oil change' matcher (both contain "oil"), double-counting one item as
// two categories and letting a response that DROPPED oil change but kept
// oil filter pass silently.
const CATEGORY_MATCHERS: Record<(typeof REQUIRED_CATEGORIES)[number], (normalizedName: string) => boolean> = {
  'oil change': (n) => n.includes('oil') && !n.includes('filter'),
  'oil filter': (n) => n.includes('oil') && n.includes('filter'),
  'air filter': (n) => n.includes('air') && n.includes('filter'),
  'cabin filter': (n) => n.includes('cabin') && n.includes('filter'),
  'brake pads': (n) => n.includes('brake'),
...
export async function selftest(): Promise<boolean> {
  console.log('[--selftest] Dry run: checking a hand-built valid schedule and a deliberately broken one.\n');
  ...
  const ok = validFailures.length === 0 && caughtMissingTyres && caughtBatteryKm;
  console.log(ok ? '\n[--selftest] OK — assertions pass a valid schedule and catch a broken one.' : '\n[--selftest] FAILED — assertions did not discriminate correctly.');
  return ok;
}
